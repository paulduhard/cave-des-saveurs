...
{#if slice.variation === 'default'}
	<section
		...
		class="bg-secondary py-6 text-center md:py-4"
	>
		<div
			class="mx-auto
 px-4 sm:px-6 md:flex md:flex-wrap md:items-baseline md:justify-center md:gap-4 lg:px-8"
		>
			{#if slice.primary.prompt_text}
				<div class="text-lg max-sm:mb-4 md:text-xl">
			...
			<form
				...
				class="flex w-80 justify-center max-sm:mx-auto md:w-96"
			>
				...
				>
					<input
						...
						required
						class="flex-grow rounded-l-md border-none bg-secondary px-4 focus:outline-none focus:ring-0"
					/>
					<button
						type="submit"
...
{:else if slice.variation === 'newsletterSignupXl'}
	<section
		data-slice-type={slice.slice_type}
		data-slice-variation={slice.variation}
 ...
 px-4 sm:px-6 md:flex md:flex-wrap md:items-baseline md:justify-center md:gap-4 lg:px-8"
		>
			{#if slice.primary.prompt_text}
				<div class="text-xl max-sm:mb-4 md:text-4xl">
					<PrismicRichText field={slice.primary.prompt_text} />
				</div>
			{/if}
