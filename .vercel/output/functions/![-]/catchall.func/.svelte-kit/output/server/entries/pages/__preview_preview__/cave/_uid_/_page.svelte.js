...
const WineCard = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="group flex h-full flex-col font-light"><a${add_attribute("href", `/vin/${wine.uid}`, 0)} class="flex flex-grow flex-col items-start border-b border-primary p-4 transition-shadow duration-300 ease-in-out hover:shadow-xl md:rounded-md md:border-none"><div class="flex flex-row-reverse md:flex-col"><div class="w-1/2 md:w-auto">${wine.image.url ? `${validate_component(PrismicImage, "PrismicImage").$$render($$result, { field: wine.image, class: "self-center" }, {}, {})}` : `<div class="bg-red-200 relative flex h-full w-full items-center justify-center rounded-lg" data-svelte-h="svelte-1k7h8j4"><img src="/assets/placeholder.png" alt="" class="opacity-50"> <span class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 -rotate-45 transform text-xl">Photo à venir</span></div>`}</div> <div class="w-1/2 md:w-auto"><span class="mt-8 font-span">${validate_component(PrismicRichText, "PrismicRichText").$$render($$result, { field: wine.title }, {}, {})}</span> <span class="mb-2 font-span text-xl">${escape(wine.domaineName)}</span> ${validate_component(PrismicRichText, "PrismicRichText").$$render($$result, { field: wine.resume }, {}, {})} <div class="mt-auto pt-4" data-svelte-h="svelte-jhncxz"><button class="inline-block border border-primary px-8 py-2 font-light text-primary transition-all duration-700 group-hover:bg-primary group-hover:text-secondary">Découvrir</button></div></div></div></a></div>`;
});
...
const Aside = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<aside class="bg-gray-100 md:w-1/4"> <div class="mb-6 border-t border-primary"><button class="mb-2 mt-4 flex w-full items-center justify-between text-left text-xl uppercase"${add_attribute("aria-expanded", isAppellationSectionExpanded, 0)} aria-controls="appellation-list">NOS VINS PAR APPELLATIONS
  ...
  })}</div>` : `<p class="text-gray-500 text-sm">${escape(filterData?.selectedDomain ? "Aucune appellation disponible pour ce domaine" : "Sélectionnez un domaine pour voir les appellations")}</p>`}</div></div>  <div class="mb-6 border-t border-primary"><button class="mb-2 mt-4 flex w-full items-center justify-between text-left text-xl uppercase"${add_attribute("aria-expanded", isDomainSectionExpanded, 0)} aria-controls="domain-list">NOS VINS PAR DOMAINES
			${validate_component(ArrowIcon, "ArrowIcon").$$render(
    $$result,
    {
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    ...
    $$rendered = `<div class="container mx-auto mt-12"><header class="mx-12 flex flex-grow items-center justify-between"><h1 class="mb-4 font-span text-5xl font-bold transition-all duration-500 ease-in-out">${escape(regionData?.region || "Region")}</h1> <button class="duration-600 hidden h-12 border border-primary px-20 font-light text-primary transition-all hover:bg-primary hover:text-secondary md:block" data-svelte-h="svelte-1aclc0b">Alcools et spiritueux</button></header> <div class="md:flex">${validate_component(Aside, "Aside").$$render(
      ...
      {}
    )} <main class="w-3/4 md:mx-6"><p class="mb-4 font-span text-lg font-bold transition-all duration-500 ease-in-out md:mx-12">${escape(regionData?.description?.[0]?.text || "")}</p>      <div class="my-12">${Object.keys(wineResults).length > 0 ? `<div class="mb-8 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">${each(wineResults, (wine) => {
